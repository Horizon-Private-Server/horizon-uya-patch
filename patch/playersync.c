...
void playerSyncHandlePlayerState(Player* player)
{
  ...
  // data->Pad[3] = 0xFF; // r2 pad is really weird, force it here if its pressed
  // R2 fix works, need to fix X/L1, L2, triangle, square, 
  // all the buttons are in rdata[3] of pad, r2 is 1111101, so to isolate it move padbits 9 to right
  // L2 is >> 8
  // L1 is >> 10
  // x is >> 14
  // triangle is >> 12
  ...
  /*
  int r2_not_pressed = stateInterpolated.PadBits >> 9 & 1;
  if (r2_not_pressed) {
    data->Pad[3] = 0xFF;
  } else {
    data->Pad[3] = 0xFD;
  ...
  //data->Pad[3] = stateInterpolated.PadBits >> 8;
  */
 data->Pad[3] = (stateInterpolated.PadBits >> 8) & data->Pad[3];
