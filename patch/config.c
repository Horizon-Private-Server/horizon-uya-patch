...
void onMenuUpdate(int inGame)
{
	...
	else if (!inGame)
  {
    // if in Online Lobby, and SubPointer equals zero (not on find game)
		if ((uiGetPointer(UIP_ONLINE_LOBBY) == uiGetActivePointerSlot(0)) && !uiGetActivePointerSlot(1))
		{
			// render message
...
void onConfigUpdate(void)
{
  ...
  if (gameSettings && gameSettings->GameLoadStartTime < 0 && netGetLobbyServerConnection())
  {
    ...
    u32 * stagingUiElements = (u32*)((u32)uiGetPointer(UIP_STAGING) + 0x110);
    u32 * stagingDetailsUiElements = (u32*)((u32)uiGetPointer(UIP_STAGING_SECONDARY_PLAYER_OPTIONS) + 0x110);
